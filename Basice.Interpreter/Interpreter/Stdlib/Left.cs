...
namespace Basice.Interpreter.Interpreter.Stdlib
{
    public class Left : ICallable
    {
        ...
        public object Call(Interpreter interpreter, Token token, List<object> arguments)
        {
            ...
            {
                throw new RuntimeException("LEFT$ argument 1 must be a string.", token);
            }
            if (!(arguments[1] is double))
            {
                throw new RuntimeException("LEFT$ argument 2 must be a number.", token);
            }
